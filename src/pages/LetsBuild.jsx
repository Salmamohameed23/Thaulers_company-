...
const CLIMATE_YEAR = 2026;

const normalizeMonthlyTemperatures = (items) => {
  if (!Array.isArray(items)) return null;

  const normalized = months.map((month, index) => {
    const item =
      items.find((entry) => entry?.month === month) || items[index] || null;
  ...
  return normalized.every((item) => item.temp !== null) ? normalized : null;
};
...
const res = await fetch(
  `${API_BASE_URL}/api/locations/climate/monthly?${params.toString()}`,
    ...
    const monthlyTemperatures = normalizeMonthlyTemperatures(
      data.data.monthlyTemperatures,
    );

    if (!monthlyTemperatures) {
      throw new Error("Incomplete monthly climate data");
    }

    update("monthlyTemperatures", monthlyTemperatures);

    setSelectedLocation({
      ...item,
      climateYear: data.data.year || CLIMATE_YEAR,
      monthlyTemperatures,
    });
  } catch (error) {
    console.error(error);
                        ...
                        {selectedLocation && (
                          <div className="mt-6 rounded-[28px] border border-black/10 bg-neutral-50 p-6 shadow-[0_18px_50px_rgba(0,0,0,0.06)]">
                            <div className="mb-5">
                              ...
                              >
                                {page.weatherTitle} — {selectedLocation.climateYear}
                              </p>
